...
export class PostgresAppUserStore {
  ...
  async list(appId?: string): Promise<AppUserRecord[]> {
    const result = appId
      ? await this.query(
          `SELECT ${appUserColumns} FROM zook_app_users WHERE app_id = $1 ORDER BY joined_at ASC`,
          [appId],
        )
      : await this.query(
          `SELECT ${appUserColumns} FROM zook_app_users ORDER BY joined_at ASC`,
        );
  ...
  async find(
    ...
    const result = await this.query(
      `SELECT ${appUserColumns} FROM zook_app_users WHERE app_id = $1 AND user_id = $2 LIMIT 1`,
      [appId, userId],
    );
