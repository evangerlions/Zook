...
export function parseLightTickTaskRow(row: DatabaseRow): LightTickTaskRow {
  return {
    ...
    estimatedMinutes: requiredInteger(row, "estimated_minutes"), scheduledFor: optionalString(row, "scheduled_business_date") ?? optionalString(row, "scheduled_for"),
    ...
    guidance: jsonObject(row, "guidance"),
