...
const CHAPTER_DRAFT_SYSTEM_PROMPT = [
  ...
  "- Meet explicit numeric Contract.scale.chapterLength.minChars/maxChars when supplied. If chapterLength.note contains a numeric range such as 2000-3000字, treat the lower number as the target chapter-length lower bound.",
  "- Do not treat a quiet setup beat as permission to write a short chapter.",
  "- Meet the target chapter length by deepening the current scene: social pressure, dialogue, bodily pain, hesitation, setting texture, resource accounting, family pressure, social friction, hidden-item routines, cautious observation, and concrete action/reaction chains.",
  "- Do not stop far below an explicit numeric minimum or numeric note range. A hard endBoundary limits what can happen next; it does not remove the duty to expand safe current-beat material.",
  ...
  "- If a boundary violation conflicts with a length or richness suggestion, the boundary wins first: remove the later-beat content, then continue expanding safe current-beat material until the target length lower bound is met.",
  "- Fix the boundary first; do not leave a chapter under the target length merely because the boundary is now clean. If a review suggestion says to delete, cut, or stop from a specific sentence/scene onward, follow that instruction literally and remove every later paragraph.",
...
> = {
  ...
  chapter_draft_review: [
    ...
    "Explicit numeric Contract.scale.chapterLength.minChars/maxChars may create a blocking length issue. If chapterLength.note contains a numeric range such as 2000-3000字, treat the lower number as the target lower bound. Use a 10% hard acceptance variance: flag drafts below minChars * 0.9 or above maxChars * 1.1, but do not block on non-numeric natural-language notes.",
