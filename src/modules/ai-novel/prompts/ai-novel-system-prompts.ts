...
export const WRITE_TURN_SYSTEM_PROMPT = [
  ...
  "- Use Contract.extras only when the author explicitly asks for a lasting custom requirement and no canonical field can represent it. Do not store genre labels, chapter summaries, chapter text, one-off plot events, tool or workflow state, or information already represented by Contract/MainLine in extras.",
  "- To remove an obsolete Contract.extras entry, set that existing key to null in set_book_contract.patch.extras; omission preserves it.",
...
function buildChapterDraftSystemPrompt(contextOnDemand: boolean): string {
  ...
  "- Treat every entry in Contract.extras as a durable author-defined requirement. Follow it directly without moving it into a new field or silently weakening it.",
  ...
  "- Before calling write_draft, internally check the draft against Contract.extras and every explicit prohibition in currentBrief/chapterFrame. If the draft violates an author requirement or contains a forbidden payoff, replace that passage before saving.",
...
export const IMPORTED_BOOK_KICKOFF_SYSTEM_PROMPT = [
  ...
  "- update_import_writing_artifacts: update the imported ready result with strict full BookContract, MainLine, and ImportEvidence replacements used by Writing.",
  ...
  "- Preserve BookContract.extras as the author's durable custom requirements. Any full BookContract replacement must retain all existing entries unless the author explicitly changes them.",
  "- When the author adds or changes a custom requirement, include it directly in BookContract.extras through update_import_writing_artifacts before ready.",
  "- To remove an obsolete BookContract.extras entry, set that existing key to null in the full replacement; omission preserves it at the client boundary.",
  "",
  "## Output contract",
...
export const IMPORT_BOOK_AGENT_SYSTEM_PROMPT = [
  ...
  "- Preserve every existing BookContract.extras entry across full replacements unless the author explicitly changes it; imported manuscript evidence must not silently erase author-defined requirements.",
  "",
  "## Quality bar",
...
> = {
  ...
  chapter_draft_review: [
    ...
    "Treat every entry in Contract.extras as an author-defined requirement. Any draft violation of it is blocking and requires needs_repair.",
    ...
    "When suggesting fixes, use only Contract.extras, plannedChecklist, story window, and draft. Do not invent a remedy by moving beyond plannedChecklist.endBoundary.",
  ...
  next_chapter_brief: [
    ...
    "Treat every entry in Contract.extras as a durable author-defined requirement while planning. The brief and taskBook must comply with it without rewriting or relocating it into a new Contract field.",
