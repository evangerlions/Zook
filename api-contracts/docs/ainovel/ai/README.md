...
- `usage` events and `done.usage` include `promptTokens`, `completionTokens`, `totalTokens`, optional provider-reported `reasoningTokens`, Zook's configured `contextWindowTokens` (default `256000`), plus the corresponding `contextUsedRatio`
