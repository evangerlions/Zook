...
lighttick.worker=new LightTickWorker(new LightTickAiRunner(lighttick.repository,{complete:async(input:any)=>{
  ...
  return {provider:"acceptance-fake",providerModel:"fake",text:JSON.stringify(clarify?{message:"已记下你的成果要求，请确认摘要后生成草案。",fields:{outcome:"可运行的小示例"}}:{tasks:[{title:"运行一个小示例并保存结果",estimated_minutes:10}]})};
}} as any));
