...
- 同一个 `clientId` 的多实例 / 多 tab 不会同时上传同一任务
...
stateDiagram-v2
  [*] --> PENDING
  PENDING --> CLAIMED: "pull-task + X-Client-Id"
...
只靠 `clientId` 只能避免“不同设备重复上传”，不能避免：
...
- claim 过期后，任务可以被同一个 `clientId` 重新领取
...
sequenceDiagram
  ...
  C->>A: GET /api/v1/logs/pull-task + X-Client-Id
