...
function modelColumns(
  ...
  return [
    ...
    { title: "Reasoning", width: 104, render: (_, row) => formatTokenNumber(row.summary.reasoningTokens) },
    { title: "未分类", width: 96, render: (_, row) => formatTokenNumber(row.summary.unclassifiedTokens) },
    { title: "单次 Token", width: 108, render: (_, row) => formatTokenNumber(row.summary.requestCount ? Math.round((row.summary.totalTokens ?? 0) / row.summary.requestCount) : undefined) },
