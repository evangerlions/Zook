...
export default function MailRoute() {
  ...
  async function handleRequestRestoreRevision(revision: number) {
    ...
    try {
      const [latestPayload, revisionPayload] = await Promise.all([
        adminApi.getEmailService(),
        adminApi.getEmailServiceRevision(revision),
      ]);
  ...
  async function handleConfirmRestoreRevision() {
    ...
    clearNotice();
    try {
      const payload = normalizeMailDocument(await adminApi.restoreEmailService(restoreRevision, restoreDesc.trim() || undefined));
      applyConfigDocument(payload);
