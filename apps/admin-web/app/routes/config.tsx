...
export default function ConfigRoute() {
  ...
  async function handleRequestRestoreRevision(revision: number) {
    ...
    try {
      const [latestPayload, revisionPayload] = await Promise.all([
        adminApi.getConfig(selectedAppId),
        adminApi.getConfigRevision(selectedAppId, revision),
      ]);
  ...
  async function handleConfirmRestoreRevision() {
    ...
    clearNotice();
    try {
      const payload = await adminApi.restoreConfig(selectedAppId, restoreRevision, restoreDesc.trim() || undefined);
      setDocument(payload);
      setValue(payload.rawJson);
      setOriginalValue(payload.rawJson);
      setDesc("");
