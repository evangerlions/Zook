...
export function normalizeOpenRouterConfigInput(value: unknown): OpenRouterConfig {
  const defaults = createDefaultOpenRouterConfig();
  if (value === undefined || value === null) {
    return defaults;
  }
  if (typeof value !== "object" || Array.isArray(value)) {
    throw new Error("openRouter 必须是 JSON object。");
  }
  ...
  if (config.transparentProxyKeyId && !KEY_ID_PATTERN.test(config.transparentProxyKeyId)) {
    throw new Error("OpenRouter 透明代理 Key ID 格式不正确。");
  }
  if (!PASSWORD_KEY_PATTERN.test(config.transparentProxyHmacSecretKey)) {
    throw new Error("OpenRouter 透明代理 HMAC Secret key 格式不正确。");
  }
  if (config.useTransparentProxy && !config.transparentProxyKeyId) {
    throw new Error("启用 OpenRouter 透明代理时必须填写 Key ID。");
  }
  return config;
}
...
function normalizeProxyBaseUrl(value: unknown): string {
  ...
  if (!normalized) {
    throw new Error("OpenRouter 透明代理 Base URL 不能为空。");
  }
  let parsed: URL;
  try {
    parsed = new URL(normalized);
  } catch {
    throw new Error("OpenRouter 透明代理 Base URL 格式不正确。");
  }
  if (
    ...
    throw new Error("OpenRouter 透明代理 Base URL 必须是无凭据、query 和 fragment 的 HTTPS URL。");
  }
  return normalized;
}
