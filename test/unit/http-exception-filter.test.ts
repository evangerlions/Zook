...
import { ApplicationError, badRequest, forbidden, internalError, unauthorized } from "../../src/shared/errors.ts";
...
test("HttpExceptionFilter converts ApplicationError to HttpResponse envelope", () => {
  const response = filter.catch(
    new ApplicationError(400, "REQ_INVALID_BODY", "Bad request", { field: "email" }),
    "req_001",
  );
...
test("HttpExceptionFilter passes through 401 errors", () => {
  let err: unknown;
  try { unauthorized("AUTH_BEARER_REQUIRED", "Token required"); } catch (e) { err = e; }
  const response = filter.catch(err as Error, "req_002");
...
test("HttpExceptionFilter passes through 403 errors", () => {
  let err: unknown;
  try { forbidden("AUTH_APP_SCOPE_MISMATCH", "Scope mismatch"); } catch (e) { err = e; }
  const response = filter.catch(err as Error, "req_003");
...
test("HttpExceptionFilter converts unknown errors to 500 SYS_INTERNAL_ERROR", () => {
  const response = filter.catch(new Error("Something went wrong"), "req_005");
...
test("HttpExceptionFilter converts non-Error objects to 500", () => {
  const response = filter.catch("string error", "req_006");
...
test("HttpExceptionFilter converts null to 500", () => {
  const response = filter.catch(null, "req_007");
  ...
  assert.throws(
    ...
    (error: unknown) => {
      if (error instanceof ApplicationError) {
        const response = filter.catch(error, "req_008");
        assert.equal(response.statusCode, 400);
        assert.equal(response.body.code, "REQ_INVALID_BODY");
        return true;
      }
...
test("HttpExceptionFilter handles internalError helper", () => {
  assert.throws(
    ...
    (error: unknown) => {
      if (error instanceof ApplicationError) {
        const response = filter.catch(error, "req_009");
        assert.equal(response.statusCode, 500);
        assert.equal(response.body.code, "SYS_INTERNAL_ERROR");
        return true;
      }
