...
function runner(repository: InMemoryLightTickRepository, output: any = {tasks:[{title:"运行示例并保存结果",estimated_minutes:10}]}) {
...
test("draft adjustment supersedes the old draft and explicit confirmation is atomic and replayable",async()=>{
  ...
  assert.deepEqual(results[0],results[1]); assert.equal(results[0].session.status,"confirmed");
...
test("expired preview cannot be confirmed",async()=>{
  const f=await setup();const started=await draft(f);await runner(f.repository).execute(owner,started.run!.id,"week_plan");
  const future=new LightTickPlanningService(f.repository,()=>new Date("2026-09-21"));
  await assert.rejects(future.command(owner,"expired","confirm",f.session.id,await confirmInput(f)),{code:"LIGHTTICK_PLANNING_STALE"});
  assert.equal((await f.repository.listTasks(owner)).length,0);
});
test("latest delivery wins and a late provider result cannot create a second draft",async()=>{
  const f=await setup();const started=await draft(f);let entered!:()=>void,release!:()=>void;
  const signal=new Promise<void>(r=>entered=r),hold=new Promise<void>(r=>release=r);
  const slow=new LightTickAiRunner(f.repository,{complete:async()=>{entered();await hold;return {text:JSON.stringify({tasks:[{title:"late",estimated_minutes:10}]})};}} as any,clock);
  const first=slow.execute(owner,started.run!.id,"week_plan");await signal;
  await runner(f.repository).execute(owner,started.run!.id,"week_plan");release();await first;
  assert.equal((await f.repository.listPlans(owner)).length,1);
  assert.equal((await f.repository.getAiRun(owner,started.run!.id))!.attemptCount,2);
});
test("oversized frozen context is rejected before provider invocation",async()=>{
  const f=await setup();const started=await draft(f);
  const saved=await f.repository.getAiRun(owner,started.run!.id);
  await f.repository.saveAiRun({...saved!,inputContext:{...saved!.inputContext,extra:"测".repeat(10000)}});
  let called=false;
  const ai=new LightTickAiRunner(f.repository,{complete:async()=>{called=true;throw new Error("unexpected");}} as any,clock);
  assert.equal((await ai.execute(owner,started.run!.id,"week_plan")).errorCode,"LIGHTTICK_PLANNING_CONTEXT_TOO_LARGE");
  assert.equal(called,false);assert.equal((await f.service.get(owner,f.session.id)).activeRunId,undefined);
});
test("configured provider routes are honored and invalid output never becomes a draft",async()=>{
  const f=await setup();const started=await draft(f);let model="";
  const {LIGHTTICK_AI_SCENES}=await import("../../src/modules/lighttick/ai/lighttick-ai-scenes.ts");
  const ai=new LightTickAiRunner(f.repository,{complete:async(input:any)=>{model=input.modelKey;return {text:JSON.stringify({tasks:[{title:"too much",estimated_minutes:999}]})};}} as any,clock,
    async name=>({...LIGHTTICK_AI_SCENES[name],modelAlias:"configured-model"}));
  assert.equal((await ai.execute(owner,started.run!.id,"week_plan")).status,"failed");
  assert.equal(model,"configured-model");assert.equal((await f.repository.listPlans(owner)).length,0);
});
test("terminal worker failure clears the active run and allows an explicit retry",async()=>{
  const f=await setup();const started=await draft(f);
  await f.service.failRun(owner,started.run!.id);
  assert.equal((await f.service.get(owner,f.session.id)).activeRunId,undefined);
  assert.equal((await f.repository.getAiRun(owner,started.run!.id))!.status,"failed");
  const retry=await draft(f,"retry-after-terminal");assert.notEqual(retry.run!.id,started.run!.id);
});
