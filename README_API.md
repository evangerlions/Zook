...
27. `POST /api/v1/ai_novel/ai-output-reports` 支持 `chat_message` 与 `chapter_revision`，校验 target 字段、分类、scene、内容 hash 和长度；章节 `chapterId` 是作品内从 `0` 开始的 ID，第一章允许传 `0`；同一账号每小时最多 20 次。服务端只在受限 Admin detail 接口解密举报原文，普通列表、日志和 audit payload 不包含原文。`POST /api/v1/ai_novel/ai-output-reactions` 当前只接受章节 revision 的 `like`，并使用相同的零基 `chapterId`。
