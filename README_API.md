...
6. 一键登录接口：
   ...
   AINovel 的 OHOS 请求（`appId=ai_novel` 且 `sdkPlatform=ohos`）使用代码内置的鸿蒙 GeYan AppID，并从 `common.passwords` 读取 `getui.gy.ohos.app_key`、`getui.gy.ohos.app_secret`、`getui.gy.ohos.master_secret`；Android/iOS 继续使用原有 `apps[appId]` 凭据。
